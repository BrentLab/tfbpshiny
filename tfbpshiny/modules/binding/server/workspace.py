...
def binding_workspace_server(
    ...
    """
    Render the binding correlation workspace: pairwise box plots and per-regulator
    scatter plots, gated on an explicit Execute Analysis button.
    ...
    # Tracks how many box-plot pairs have completed their render_plotly call.
    # Reset to 0 when Execute is clicked; incremented (under reactive.isolate) inside
    # each _box_plot render so analysis_status can show a "building plots" message
    # during the render phase that follows the task phase.
    _boxes_rendered: reactive.Value[int] = reactive.value(0)
    _boxes_expected: reactive.Value[int] = reactive.value(0)

    # Scatter render-phase tracking — same pattern as boxes but also resets when
    # selected_reg changes so the "preparing" message reappears for each new regulator.
    ...
    def _init_selected_reg() -> None:
        ...
        computing in parallel with box-plot renders rather than waiting for the
        ``regulator_selector`` UI render to fire.
    ...
    def _reset_scatter_epoch() -> None:
        ...
        Only acts when the task has already succeeded. Reading ``_run_analysis``
        under ``reactive.isolate`` means this effect fires only on ``selected_reg``
        changes, not on task-status transitions.
    ...
    def analysis_status() -> ui.Tag:
        ...
        ``render_plotly`` closures have completed and returned their FigureWidgets.
        Each closure increments ``_boxes_rendered`` under ``reactive.isolate`` so
        this render re-fires without the box plots re-running.
    ...
    def _update_all_highlights() -> None:
        ...
        only the delta for trace 1 is sent to the client.
    ...
    def box_plot_container() -> ui.Tag:
        ...
        Only re-renders when the task result changes. Each slot is filled by its own
        per-pair ``render_plotly`` registered below.
        ...
        if not pairs:
            return ui.div(
                {"class": "empty-state"},
                ui.p("No active binding dataset pairs. Select at least two datasets."),
            )
    ...
    def scatter_status() -> ui.Tag:
        ...
        Covers two cases: the render phase after a fresh Execute (scatters computing
        in parallel with box renders), and regulator changes by the user.

        :trigger _scatter_rendered: re-renders as each scatter plot finishes.
        :trigger _scatter_expected: re-renders when expected count is set.
    ...
    def regulator_selector() -> ui.Tag:
        ...
        Initial selection is managed by ``_init_selected_reg`` so this render only
        reflects the current value; it does not write to ``selected_reg``.
    ...
    def _make_scatter_render(db_a: str, db_b: str, pair_idx: int) -> None:
        ...
        async def _scatter_plot() -> ui.Tag:
            ...
            if (db_a, db_b) not in pairs:
                # Not an active pair — not counted against _scatter_expected.
                return ui.span()
