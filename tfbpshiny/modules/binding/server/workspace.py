...
def binding_workspace_server(
    ...
    def _fill_corr_cache() -> None:
        ...
        makes returning to the binding tab after a tab switch free — the DOM
        recreation invalidates the effect, but the key comparison short-circuits
        before any SQL is executed, and the existing ``_corr_cache`` value
        remains unchanged, so downstream renders read the cached result without
        waiting.
        ...
        if key == cached_key:
            logger.debug("binding _fill_corr_cache: cache hit, skipping queries")
            return
    ...
    def _fill_distributions_cache() -> None:
        ...
        if key == cached_key:
            logger.debug("binding _fill_distributions_cache: cache hit, skipping build")
            return
        ...
        # per-tag lookups used by the highlight overlay in distributions_plot
    ...
    def distributions_plot() -> ui.Tag:
        ...
        logger.debug(
            f"RENDER binding/distributions_plot "
        ...
        # Add highlight overlay for the selected regulator if present.
        ...
        # post_script is injected by plotly.io.to_html immediately after the
        # Plotly.newPlot() call that initialises the figure.  Plotly replaces
        # the literal ``{plot_id}`` with the actual div ID before inserting the
        # script into the page.
        #
        # The IIFE (immediately-invoked function expression) runs once when
        # Shiny renders this output.  It looks up the graph div by that ID and
        # registers a "plotly_click" handler on it using Plotly's own event API
        # (``div.on``), not the DOM's addEventListener — Plotly emits named
        # events through its own internal bus.
        #
        # When a dot is clicked, ``data.points[0].customdata`` contains the
        # regulator locus tag stored in the trace's ``customdata`` array (set
        # above when building the highlight overlay trace).  That value is
        # pushed to the Shiny input ``selected_regulator`` with
        # ``priority: 'event'`` so Shiny flushes it immediately even if the
        # value matches the previous one (e.g. clicking the same dot twice).
        post_script = (
            "(function() {"
        ...
        with profile_span(
            ...
            html = to_html(
                fig, include_plotlyjs=False, full_html=False, post_script=post_script
            )
        logger.debug(
            f"RENDER_DONE binding/distributions_plot "
            f"#{_render_counts['distributions_plot']} "
    ...
    def _regulator_choices() -> tuple[dict[str, str], str] | None:
        ...
        that switching from the perturbation tab preserves the last selection.
    ...
    def _resolve_active_regulator() -> None:
        ...
        is ignored so that a cross-tab selection made on the perturbation page
        is never overwritten by a spurious DOM-reset event here.
        ...
        if not current or current not in choices:
            logger.debug(f"binding _resolve_active_regulator: ignoring {current!r}")
            return
        if shared_regulator is not None:
            ...
            if current != old_val:
                logger.debug(
                    f"binding _resolve_active_regulator: {old_val!r} -> {current!r}"
                )
    ...
    def _sync_regulator_choices() -> None:
        ...
        # If not (e.g. a perturbation-only TF was selected on the other tab,
        # or shared_regulator is still empty on first mount), fall back to the
        ...
        logger.debug(f"binding _sync_regulator_choices: pushing selected={effective!r}")
    ...
    def _fill_scatter_cache() -> None:
        ...
        :trigger active_module: exits early (without queries) when binding is     not
        the active tab; re-fires when the user switches to binding. :trigger
        ...
        # re-fires when the user switches TO binding and can fill a stale cache.
        if active_module is not None and active_module() != "binding":
            return
        ...
        # This prevents running 6 scatter queries for a cross-tab regulator that
        ...
        if key == cached_key:
            logger.debug("binding _fill_scatter_cache: cache hit, skipping queries")
            return
    ...
    def regulator_plots() -> ui.Tag:
        ...
        logger.debug(
            f"RENDER binding/regulator_plots "
        ...
        logger.debug(
            f"RENDER_DONE binding/regulator_plots "
            f"#{_render_counts['regulator_plots']} "
            f"elapsed={time.perf_counter()-t0:.3f}s"
        )
        return result
